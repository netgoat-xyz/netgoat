...
export function AuthForm({ className, ...props }: React.ComponentProps<"div">) {
  ...
  async function handleLogin(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await axios.post(process.env.backendapi+"/api/auth/login", { email, password })
      if (res.data.jwt) {
        localStorage.setItem("jwt", res.data.jwt)
        // Optionally decode and store user info
        const user = jwtDecode(res.data.jwt)
        // You can also set user in context or state here
  ...
  async function handleRegister(e: React.FormEvent<HTMLFormElement>) {
    ...
    const password = (form.elements.namedItem("password") as HTMLInputElement)?.value
    try {
      await axios.post(process.env.backendapi+"/api/auth/register", { username, email, password })
      setMode("login")
      setDirection(-1)
    } catch (err: any) {
  ...
  return (
    <div className={cn("flex flex-col w-full gap-6 items-center justify-center min-h-screen", className)} {...props}>
        ...
        <CardContent className="flex flex-col p-0 min-h-0 w-full items-center justify-center">
          <div className="p-8 flex flex-col justify-center w-full items-center">
            <div className="flex flex-col gap-10 w-full items-center">
              <div className="flex flex-col items-center min-h-[90px] w-full">
                  ...
                  {mode === "login" ? (
                    ...
                    >
                      ...
                      <div className="text-center text-sm mt-2">
                        ...
                        <button type="button" className="underline underline-offset-4" onClick={() => switchMode("register")}>Sign up</button>
                      </div>
                    </motion.form>
                  ) : (
                    <motion.form
                    ...
                    >
                      ...
                      <div className="text-center text-sm mt-2">
                        ...
                        <button type="button" className="underline underline-offset-4" onClick={() => switchMode("login")}>Sign in</button>
                      </div>
                    </motion.form>
                  )}
                </AnimatePresence>
